...
| Table                | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `verseMemory`        | One row per (`userId`, `verseRefId`) tracking the live schedule: `status` (`new`/`learning`/`reviewing`/`mastered`), `learnStage` (0..3), `stageReps` (exact reps banked at the current band; see [Fade dial](#the-fade-dial-learning-bands)), `ease`, `intervalDays`, `dueAt`, `consecutiveCorrect`, `lapses`, `lastReviewedAt?`, `createdAt`. Indexed `by_userId_dueAt`, `by_userId_status`, `by_userId_verseRefId`. |
...
**Reps + close/off handling.** `stageReps` counts the **exact** reps banked at the current band. A band only clears once `stageReps` reaches its `requiredReps`; on clearing, the verse advances to the next band (`stageReps` resets to 0), and clearing the last band (`From Memory`) **graduates** it into the reviewing phase (`status: "reviewing"`, a fresh 1-day interval). Grading drives the dial via `classifyVerseAttempt`:
...
**Graduation naming.** A verse is considered **learned** once it reaches `reviewing` (it cleared `From Memory` and now recalls from hidden on a growing interval), and **mastered** once its interval reaches `MASTERED_INTERVAL_DAYS` (30 days). "Learning" is the dial; "reviewing"/"mastered" are the spaced-repetition tail.
...
Alongside Review, Memory home exposes a **Practice** surface — a focused, **self-directed** drill over a chosen verse set. Where Review plays the due queue on the scheduler's terms, Practice lets the learner pick _what_ order to work through, with no due-date gating. The masking rung is **not** hand-picked: Practice rides the same [fade dial](#the-fade-dial-learning-bands) as the learn ladder, showing each verse at its live band.
...
- **Set snapshot.** The practice set is **captured once when the board opens** (a `useState` initializer over the initial `verses` prop, mirroring how `ReviewPlayer` freezes its due queue). Mid-session heart changes therefore can't mutate the active set or reorder it under the current index — most importantly under Shuffle. The rail's per-verse progress dots still read **live** schedule state so they update as reps bank.
- **Motion & a11y.** The card entrance uses Framer Motion and honors `prefers-reduced-motion` (via `useReducedMotion`); the rail and Shuffle toggles are `aria-pressed` buttons.
...
The ring fraction is a pure function of the verse's memory state (`masteryRingFraction`, `src/lib/mastery-ring.ts`, unit-tested). During learning the ring **grows with the [fade dial](#the-fade-dial-learning-bands)** — each of the four bands owns an equal slice of the learning arc, and `stageReps` fills the current band's slice — so the reader's heart reflects rep-by-rep progress even outside the learn cards, then hands off continuously to the reviewing arc at 0.5:

| Status      | Ring                                                                                                                                                                         |
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `new`       | 0 — no ring ("saved, not yet started")                                                                                                                                       |
| `learning`  | 0 → just under 0.5, linear in `(learnStage + stageReps / requiredRepsFor(learnStage)) / (MAX_LEARN_STAGE + 1)` (required reps sourced from `SUPPORT_BANDS`, never hardcoded) |
| `reviewing` | 0.5 → 0.9, scaled linearly by `intervalDays` from just-graduated up to `MASTERED_INTERVAL_DAYS`                                                                              |
| `mastered`  | 1 (full)                                                                                                                                                                     |

`masteryRingFraction(status, intervalDays, learnStage?, stageReps?)` stays pure (no React, no `Date.now()`); `learnStage` / `stageReps` default to `0` so legacy rows and non-learning statuses (whose fraction ignores them) behave exactly as before. The `reviewing`/`mastered` outputs are unchanged.

**Avoiding N+1 for the rings.** The reader does not add a per-verse query. `savedVerses.listForChapter` — the single query the reader already runs per chapter — now joins each saved verse's memory row into an optional `memory` field (including `learnStage` + `stageReps`). `PassageViewBody` reads that back through `savedSpans` and computes the ring fraction with `masteryRingFraction` for the exact `(startVerse, endVerse)` span backing each heart control, passing it as `PassageHeartControl.masteryFraction`.
...
- Study and Memory are both reachable from the app toolbar (`TabBar`), which highlights when the active route starts with `/study` or `/memory`. The toolbar **Memory** (`Brain`) and **Study** (`BookOpen`) icons are **gated fallbacks**: each shows once its reveal predicate is reached (or the reveal has been acknowledged) and completes its reveal hint on open, mirroring the Search button. The primary reveal callouts live on the Mode Dock; the toolbar icons keep the modes reachable when the dock is off.
...
A single floating pill, bottom-center, is the app's only new persistent chrome. It has up to three segments — **Notes**, **Memory**, and **Study** (in that order) — and is `position: fixed`, so it adds **zero layout cost** and stays thumb-reachable on mobile.

- **Progressive reveal.** Notes is always available; **Memory** unlocks on the **first heart** (`shouldRevealMemory` → `heartsCount >= MEMORY_MIN_HEARTS = 1`) and **Study** once the library has **enough notes** (`shouldRevealStudy` → `notesCount >= STUDY_MIN_NOTES = 4`). The dock computes an **unlocked-mode count** from these predicates and **renders nothing until at least two modes are unlocked** — a lone Notes segment isn't worth the chrome, so a brand-new account sees no dock at all until it earns its first heart (Notes + Memory) or fourth note (Notes + Study).
- **Navigation.** "Notes" routes to the passage workspace (`/passage/$passageId` using the last-viewed passage from `useTabs().backPassageId`); "Memory" routes to `/memory`; "Study" routes to `/study`. The active segment reflects the current route (`aria-current="page"`). _Simplification:_ rather than tracking a distinct "last passage" concept, Notes reuses the existing `backPassageId` (the most recent passage tab), which is the app's default passage target.
- **Live badge.** The **Memory** segment shows `verseMemory.dueCount({ now })` (with `now` from `useLiveNow()`), so it updates as verses fall due or get reviewed. The badge is **absent entirely when the count is zero** (never cry wolf) and carries an `aria-label` like "7 verses due for review".
...
- **Keyboard.** `⌘J` / `Ctrl+J` **cycles the unlocked modes** in dock order (Notes → Memory → Study → Notes), skipping any mode that hasn't been revealed yet. It's wired the same ad-hoc way `TabBar` wires `⌘G` (a document `keydown` listener); tooltips use `formatCommandOrControlShortcut`.
...
- **Reveal callouts.** Each new mode has its own one-time reveal, reusing `useFeatureHint` + `FeatureCallout`: **Memory** uses `FEATURE_HINTS.MEMORY_REVEAL_AFTER_FIRST_HEART` (the "it's now in Memory" callout on the Memory segment) and **Study** uses `FEATURE_HINTS.STUDY_REVEAL_AFTER_NOTES`. The dock forces itself visible while either callout is pending. The dock only **claims** a hint in the global display queue while it can actually render the callout (i.e. `modeDock !== "off"`); when the dock is off, the toolbar Memory/Study links are the fallbacks that **complete** the reveal on open, so the queue is never pinned by an eligible-but-unrendered callout. `complete()` is idempotent, so whichever path the user takes (dock callout or toolbar) resolves the hint exactly once. _(The legacy `STUDY_REVEAL_AFTER_FIRST_HEART` id is retained in `feature-hints.ts` so persisted dismissals stay valid, but it is no longer surfaced.)_
- **First-open explainer.** The first time Memory is opened, `MemoryFirstOpenDialog` (`src/components/memory/memory-first-open-dialog.tsx`, mounted in `MemoryHomePage`) shows a one-time orientation dialog ("Heart verses to add them here · Review what's due · Make packs to focus"), mirroring `StudyModeExplainerDialog`. It uses `FEATURE_HINTS.MEMORY_FIRST_OPEN_EXPLAINER` and bypasses the display queue (destination help, not a dock reveal).
- **Preference.** `userSettings.modeDock` (`"auto-hide"` | `"always"` | `"off"`, default `"auto-hide"`) persists the behavior via `userSettings.getModeDockPreference` / `setModeDockPreference`. `"always"` disables auto-hide (dock always visible); `"off"` hides the dock entirely (the toolbar Memory/Study icons remain the escape hatch). A control in Settings (`ModeDockSection`) toggles it. Only this preference is persisted — scroll/focus visibility is local component state.
