...
function buildBookAliasEntries(): Array<{ alias: string; name: string }> {
  ...
  for (const book of BIBLE_BOOKS) {
    ...
    const aliasCandidates = additionalAliases[book.name.replace(/\s/g, "")] ?? []
...
export function resolveCanonicalBookName(input: string): string | null {
  ...
  const exact = BOOK_ALIAS_ENTRIES.find((entry) => entry.alias === normalizedInput)
  if (exact) return exact.name

  const prefix = BOOK_ALIAS_ENTRIES.find((entry) => entry.alias.startsWith(normalizedInput))
...
export function searchBibleBooks(input: string, limit = 8): BookMatch[] {
  ...
  for (const book of BIBLE_BOOKS) {
    const aliases = new Set(
      BOOK_ALIAS_ENTRIES.filter((entry) => entry.name === book.name).map((entry) => entry.alias)
    )
...
export function parseVerseRef(str: string, options: ParseVerseRefOptions = {}): VerseRef | null {
  const value = str.trim()
  if (!value) return null

  const compactReferenceMatch = value.match(/^(\d+)(?::(\d+)(?:-(\d+))?)?$/)
  if (compactReferenceMatch && options.defaultBook) {
    const chapter = compactReferenceMatch[2] ? Number.parseInt(compactReferenceMatch[1], 10) : options.defaultChapter
...
export function buildVerseSuggestions(
  ...
  return results.filter(
    (item, index, list) => index === list.findIndex((candidate) => candidate.key === item.key)
  )
}
